...
int main (int argc, char** argv)    {
    ...
    //Input XML file
    std::string input {"olala<4234&^<7847238mamaa 4num=\"32\"4\"11\"\"42\"\"\" 56just=\" 4killed=\" 128aman=\" 256put=\" 512agun=\"90 against=\"><papaa><dadaa> bla bla bla    ><<<<<        <a></a>    bla bla bla    ><<<<<          </dadaa>      <test>  </test>     </43264826492837468papaa>   <b></b>        </mamaa>"};
    
    //For minifiying XML input
    std::string output_mini {};

    //For prettifying XML output
    std::string output_pretty {};
    std::string tab {"\t"};
    int indentator{0};
    ...
    while((otag_flag = std::regex_search(input,otag_match,otag)) + std::regex_search(input,ctag_match,ctag))   {
        ...
        if(otag_match.position(1) < ctag_match.position(1) && otag_flag != 0)   {
            ...
            **/
            ...
            output_mini += opening_tag;
            output_pretty += ((tab * indentator) + opening_tag + "\n");
            indentator ++;
        ...
        //Closing tag found
            ...
            **/
            ...
            output_mini += text.substr(i,(j-i+1));

            if(text.substr(i,j) != "")  
                output_pretty += ((tab * indentator) + text.substr(i,(j-i+1)) + "\n");
            indentator --;
            ...
            **/
            ...
            try {
                if((tags_order.top()) == (closing_tag.substr(2,((closing_tag.size())-3))))    {
                    tags_order.pop();
                    output_mini += closing_tag;
                    output_pretty += ((tab * indentator) + closing_tag + "\n");
    ...
    //Extra line removed
    output_pretty = output_pretty.substr(0,(output_pretty.size()-1));
